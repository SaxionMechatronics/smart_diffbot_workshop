...
std::string
ExactPathNavigator::getDefaultBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  std::string default_bt_xml_filename;
  auto node = parent_node.lock();

  if (!node->has_parameter("default_nav_exact_path_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("smart_diffbot_navigation");
    node->declare_parameter<std::string>(
      "default_nav_exact_path_bt_xml",
      pkg_share_dir +
      "/behavior_trees/navigate_exact_path.xml");
  }

  node->get_parameter("default_nav_exact_path_bt_xml", default_bt_xml_filename);

  return default_bt_xml_filename;
}

bool
ExactPathNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
...
void
ExactPathNavigator::goalCompleted(
...
void
ExactPathNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
...
void
ExactPathNavigator::initializeGoalPoses(ActionT::Goal::ConstSharedPtr goal)
{
  ...
  geometry_msgs::msg::PoseStamped test = goal->poses[0];
  blackboard->set<Goals>(goals_blackboard_id_, goal->poses);
  blackboard->set<Goal>(goal_blackboard_id_, goal->poses[0]);
}

...
std::string
DockingNavigator::getDefaultBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  std::string default_bt_xml_filename;
  auto node = parent_node.lock();

  if (!node->has_parameter("default_docking_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("smart_diffbot_navigation");
    node->declare_parameter<std::string>(
      "default_docking_bt_xml",
      pkg_share_dir +
      "/behavior_trees/navigate_docking.xml");
  }

  node->get_parameter("default_docking_bt_xml", default_bt_xml_filename);

  return default_bt_xml_filename;
}

bool
DockingNavigator::cleanup()
{
  self_client_.reset();
  return true;
}
...
bool
DockingNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
...
void
DockingNavigator::goalCompleted(
...
void
DockingNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
...
void
DockingNavigator::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
  geometry_msgs::msg::PoseStamped current_pose;
  nav2_util::getCurrentPose(
    current_pose, *feedback_utils_.tf,
...
void
DockingNavigator::onGoalPoseReceived(const geometry_msgs::msg::PoseStamped::SharedPtr pose)
{
  ActionT::Goal goal;
  goal.pose = *pose;

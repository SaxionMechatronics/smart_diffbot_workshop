...
#include "smart_diffbot_navigation/navigator_plugins/bt_navigator.hpp"
...
: nav2_util::LifecycleNode("smart_diffbot_navigation", "", options)
{
...
BtNavigator::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  pose_navigator_ = std::make_unique<smart_diffbot_navigation::NavigateToPoseNavigator>();
  poses_navigator_ = std::make_unique<smart_diffbot_navigation::NavigateThroughPosesNavigator>();
  exact_path_navigator_ = std::make_unique<smart_diffbot_navigation::ExactPathNavigator>();
  docking_navigator_ = std::make_unique<smart_diffbot_navigation::DockingNavigator>();

  smart_diffbot_navigation::FeedbackUtils feedback_utils;
  ...
  if (!exact_path_navigator_->on_configure(
      shared_from_this(), plugin_lib_names, feedback_utils, &plugin_muxer_, odom_smoother_))
  {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
BtNavigator::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_activate() || 
      ...
      !exact_path_navigator_->on_activate() ||
...
BtNavigator::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_deactivate() || 
      ...
      !exact_path_navigator_->on_deactivate()  ||
...
BtNavigator::on_cleanup(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_cleanup() || 
      ...
      !exact_path_navigator_->on_cleanup() ||
...
RCLCPP_COMPONENTS_REGISTER_NODE(smart_diffbot_navigation::BtNavigator)
